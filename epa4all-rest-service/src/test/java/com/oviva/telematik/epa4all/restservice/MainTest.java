...
class MainTest {
  ...
  @Test
  void buildClientService_setsProxyEnvironmentAndTelematikId() throws Exception {
    var ks = createEmptyPkcs12(tempDir.resolve("keys-client-service.p12"), "pw");

    var cfg =
        mapProvider(
            Map.ofEntries(
                Map.entry("konnektor.uri", "https://konn:7443"),
                Map.entry("credentials.path", ks.toString()),
                Map.entry("credentials.password", "pw"),
                Map.entry("proxy.address", "proxy.host"),
    ...
    var service = invokeBuildClientService(main, config);

    var proxyField = Epa4allClientService.class.getDeclaredField("tiProxyAddress");
    proxyField.setAccessible(true);
    var proxy = (InetSocketAddress) proxyField.get(service);

    var envField = Epa4allClientService.class.getDeclaredField("environment");
    envField.setAccessible(true);
    var env = (Environment) envField.get(service);

    var tidField = Epa4allClientService.class.getDeclaredField("telematikId");
    tidField.setAccessible(true);
    var tid = (String) tidField.get(service);

    assertEquals(new InetSocketAddress("proxy.host", 9999), proxy);
    assertEquals(Environment.RU, env);
    assertEquals("TID-XYZ", tid);
  }

  @Test
  void buildClientService_setsNullProxyWhenNotConfigured() throws Exception {
    var ks = createEmptyPkcs12(tempDir.resolve("keys-client-service-noproxy.p12"), "pw");
  ...
  private static Main.Config invokeLoadConfig(Main main, ConfigProvider provider) throws Exception {
    Method m =
  ...
  private static Epa4allClientService invokeBuildClientService(Main main, Main.Config cfg)
      throws Exception {
    Method m = Main.class.getDeclaredMethod("buildClientService", cfg.getClass());
    m.setAccessible(true);
    return (Epa4allClientService) m.invoke(main, cfg);
  }
